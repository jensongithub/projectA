...
class MY_Router extends CI_Router {
	...
	function _set_routing()
	{
		...
		if( $_SERVER["SERVER_NAME"] == CN_DOMAIN ){
			if (defined('ENVIRONMENT') AND is_file(APPPATH.'config/'.ENVIRONMENT.'/routes_cn.php'))
			{
				include(APPPATH.'config/'.ENVIRONMENT.'/routes_cn.php');
				$routes_loaded = true;
				if( ! defined('IS_CN' ) )
					define( 'IS_CN', 1 );
			}
			elseif (is_file(APPPATH.'config/routes_cn.php'))
			{
				include(APPPATH.'config/routes_cn.php');
				$routes_loaded = true;
				if( ! defined('IS_CN' ) )
					define( 'IS_CN', 1 );
			}
